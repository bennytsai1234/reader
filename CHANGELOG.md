...
- 修正閱讀器正文上方仍額外保留空白的問題，現在正文直接貼齊頂部閱讀區
- 修正閱讀器「上一章」會落到上一章末尾的行為，現在會回到上一章第一列
- 移除界面設定中未完成的「自定義背景圖」入口，避免誤導
- 修正白天 / 夜間主題切換仍會輕微跳頁的問題，主題切換不再觸發整套重排版
- 修正上下滾動模式下頁碼與閱讀百分比不準確的問題，現在依據實際閱讀位置計算
- 修正書源管理頁全選後底部三點選單被擠壓而難以點擊的問題
- 修正 `酷我小說` 等重型 JS 發現規則在書源校驗時的崩潰風險，現在會序列化執行並在 timeout / error 後釋放 runtime

### 體驗優化

- 自動翻頁重構為兩條明確路徑：`平移翻頁` 使用逐頁掃描線節奏，`上下滾動` 改為在 scrollable 上連續位移
- 自動翻頁現在會在到達書尾時正確停止，切換模式或調整速度時也會重啟計時週期
- 自動翻頁面板中的模式命名改得更清楚，直接對應 `平移翻頁` / `上下滾動`

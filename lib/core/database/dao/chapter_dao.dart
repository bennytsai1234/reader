...
class ChapterDao extends DatabaseAccessor<AppDatabase> with _$ChapterDaoMixin {
  ...
  Future<void> insertChapters(List<BookChapter> chapterList) async {
    await batch((b) => b.insertAllOnConflictUpdate(chapters, chapterList.map((e) => BookChapterToInsertable(e).toInsertable()).toList()));
  }
  ...
  Future<void> updateContent(String url, String content) {
    return (update(chapters)..where((t) => t.url.equals(url)))
  ...
  Future<bool> hasContent(String url) async {
    final c = await (select(chapters)..where((t) => t.url.equals(url))).getSingleOrNull();
  ...
  Future<void> saveContent(String url, String content) => updateContent(url, content);

  Future<void> deleteContentByBook(String bookUrl) {
    return (update(chapters)..where((t) => t.bookUrl.equals(bookUrl)))

...
class _BookmarkListPageState extends State<BookmarkListPage> {
  ...
  void initState() {
    super.initState();
    _loadBookmarks();
    _eventSub = AppEventBus()
        .onName('up_bookmark')
        .listen((_) => _loadBookmarks(_searchController.text));
  }
  ...
  Future<void> _loadBookmarks([String searchKey = '']) async {
    ...
    final bookmarks =
        searchKey.isEmpty
            ? await _bookmarkDao.getAll()
            : await _bookmarkDao.search(searchKey);
  ...
  Future<void> _exportBookmarks({bool asJson = false}) async {
    ...
    final content =
        asJson
            ? jsonEncode(_allBookmarks)
            : _allBookmarks.map((e) => e.bookText).join('\n');
    await SharePlus.instance.share(
      ShareParams(text: content, subject: 'Legado 書籤匯出'),
    );
  }
    ...
    final res = await showModalBottomSheet<int>(
      ...
      builder:
          (ctx) => Column(
            ...
            children: [
              ListTile(
                title: const Text('文字版'),
                onTap: () => Navigator.pop(ctx, 0),
              ),
              ListTile(
                title: const Text('JSON版'),
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  await _bookmarkDao.upsert(
                    bookmark.copyWith(content: ctrl.text),
                  );
                  if (ctx.mounted) {
                    Navigator.pop(ctx);
                    _loadBookmarks();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('書籤筆記'),
        actions: [
          IconButton(
            icon: Icon(_groupByBook ? Icons.list : Icons.folder),
            ...
            child:
                _isLoading
                    ...
                    : ListView.builder(
                      ...
                      itemBuilder:
                          (context, i) => ListTile(
                            title: Text(_allBookmarks[i].chapterName),
                            subtitle: Text(
                              _allBookmarks[i].bookText,
                              maxLines: 1,
                            ),
                            onTap: () => _editBookmark(_allBookmarks[i]),
                          ),
                    ),
          ),
        ],
